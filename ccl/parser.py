...
class Parser(CCLVisitor):
    def __init__(self) -> None:
        super().__init__()
        self._name_handling: VarContext = VarContext.STORE
    ...
    def visitMethod(self, ctx: CCLParser.MethodContext) -> Method:
        annotations = []
    ...
    def visitParameterAnnotation(self, ctx: CCLParser.ParameterAnnotationContext) -> ParameterAnnotation:
    ...
    def visitABAnnotation(self, ctx: CCLParser.ABAnnotationContext) -> ObjectAnnotation:
        name = Name(self.get_pos(ctx.name), ctx.name.text, VarContext.ANNOTATION)
    ...
    def visitAndOrConstraint(self, ctx: CCLParser.AndOrConstraintContext) -> BinaryLogicalOp:
    ...
    def visitNotConstraint(self, ctx: CCLParser.NotConstraintContext) -> UnaryLogicalOp:
        constraint = self.visit(ctx.constraint())
        return UnaryLogicalOp(self.get_pos(ctx), UnaryLogicalOp.Ops.NOT, constraint)

    def visitParenConstraint(self, ctx: CCLParser.ParenConstraintContext) -> Constraint:
        return self.visit(ctx.constraint())

    def visitCompareConstraint(self, ctx: CCLParser.CompareConstraintContext) -> RelOp:
        lhs = self.visit(ctx.left)
        rhs = self.visit(ctx.right)
        op = RelOp.Ops(ctx.op.text)
        return RelOp(self.get_pos(ctx), lhs, op, rhs)

    def visitPredicateConstraint(self, ctx: CCLParser.PredicateConstraintContext) -> Predicate:
        name = ctx.pred.text
    ...
    def visitExprAnnotation(self, ctx: CCLParser.ExprAnnotationContext) -> ExprAnnotation:
    ...
    def visitAssign(self, ctx: CCLParser.AssignContext) -> Assign:
    ...
    def visitBinOp(self, ctx: CCLParser.BinOpContext) -> BinaryOp:
        left = self.visit(ctx.left)
        right = self.visit(ctx.right)
        op = BinaryOp.Ops(ctx.op.text)
        return BinaryOp(self.get_pos(ctx.op), left, op, right)

    def visitParenOp(self, ctx: CCLParser.ParenOpContext) -> Expression:
        return self.visit(ctx.expr())

    def visitNumber(self, ctx: CCLParser.NumberContext) -> Number:
        try:
            n = int(ctx.getText())
        except ValueError:
            n = float(ctx.getText())
    ...
    def visitBasename(self, ctx: CCLParser.BasenameContext) -> Name:
        return Name(self.get_pos(ctx), ctx.name.text, self._name_handling)

    def visitSubscript(self, ctx: CCLParser.SubscriptContext) -> Subscript:
    ...
    def visitFor_loop(self, ctx: CCLParser.For_loopContext) -> For:
    ...
    def visitFor_each(self, ctx: CCLParser.For_eachContext) -> ForEach:
        name = Name(self.get_pos(ctx.identifier), ctx.identifier.text, VarContext.STORE)
    ...
    def visitPropertyAnnotation(self, ctx: CCLParser.PropertyAnnotationContext) -> Property:
        name = Name(self.get_pos(ctx.name), ctx.name.text, VarContext.ANNOTATION)
